...
public class SimpleResolver implements Resolver {
    ...
    public Message send(Message query) throws IOException {
        ...
        applyEDNS(query2);
        if (this.tsig != null) {
            this.tsig.apply(query2, (TSIGRecord) null);
        }
        ...
        while (true) {
            ...
            boolean tcp2 = tcp;
            if (tcp2) {
                in = TCPClient.sendrecv(this.localAddress, this.address, out, endTime2);
            } else {
                in = UDPClient.sendrecv(this.localAddress, this.address, out, udpSize, endTime2);
            }
            if (in.length >= 12) {
                int id = ((in[0] & NavSatStatus.STATUS_NO_FIX) << 8) + (in[1] & NavSatStatus.STATUS_NO_FIX);
                ...
                if (id != qid) {
                    ...
                    if (tcp2 || this.ignoreTruncation || !response.getHeader().getFlag(6)) {
                        return response;
                    }
                    tcp = true;
                    endTime = endTime2;
